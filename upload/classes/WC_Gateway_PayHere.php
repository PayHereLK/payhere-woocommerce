...
class WC_Gateway_PayHere extends WC_Payment_Gateway
{
    ...
    function init_form_fields()
    {
        ...
        $this->form_fields = array(
            ...
            'seperator_2' => array(
                ...
                'description' => __('You will only need below credentials if you have subscriptions or Charging API available.', 'woo_payhere'),
                'type' => 'seperator'
            ),
            // Business App ID
            'enable_tokenizer' => array(
                'title' => __('Enable Tokenizer', 'woo_payhere'),
                'type' => 'checkbox',
    ...
    function generate_payhere_form($order_id)
    {
        ...
        $hash_1 = $this->generate_hash($effective_merchant_id, $order_id, 1, get_woocommerce_currency(), $effective_merchant_secret);
        $hash = $payhere_args['hash'];
...
//            For From Submit
        ...
        $pre_approve_url = $effective_test_mode == 'yes' ? 'https://sandbox.payhere.lk/pay/preapprove' : 'https://www.payhere.lk/pay/preapprove';
...
//                          Added order total equels request amount this will reject the pre approve process,
//                          Then added if request is pre approval order total is 1
                            ...
                            if (($md5hash != $md5sig) || ($_REQUEST['payhere_amount'] != ($_order_total) && !$is_subscription)) {
                                $verified = false;
                            }
                        }
                    }
                        ...
                        if ($verified) {
                            ...
                            if ($status == "2") {
                                ...
                                if (isset($_REQUEST['customer_token'])) {
                                    ...
                                    update_user_meta($order->get_customer_id(), 'payhere_customer_token', $_REQUEST['customer_token']);
    ...
    /*
     * Charge via Payhere Chaging API
     */
        ...
        if (empty($customer_token)) {
            ...
            if (isset($auth_token_data->access_token) && !empty($auth_token_data->access_token)) {
                ...
                if ($charge_response->status == '1') {
                    ...
                    $json['type'] = 'ERR';
                    $json['message'] = 'Can\'t make the payment. Payment Failed.';
                }
            } else {
                $json['type'] = 'ERR';
    ...
    private function getAuthorizationToken($app_id, $app_secret, $sandbox = false)
    {
    ...
    private function submitCharge($token, $customer_token, $order_id, $amount, $sandbox = false)
    {
        $url = 'https://www.payhere.lk/merchant/v1/payment/charge';
        if ($sandbox == true) {
